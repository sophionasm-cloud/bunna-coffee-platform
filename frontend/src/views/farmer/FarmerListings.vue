<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
      <div class="stat-card"><p class="text-sm text-earth-500 mb-1">Total Listings</p><p class="font-display text-3xl font-bold text-earth-900">{{ listings.length }}</p></div>
      <div class="stat-card"><p class="text-sm text-earth-500 mb-1">Approved</p><p class="font-display text-3xl font-bold text-forest-700">{{ listings.filter(l=>l.status==='approved').length }}</p></div>
      <div class="stat-card"><p class="text-sm text-earth-500 mb-1">Pending</p><p class="font-display text-3xl font-bold text-gold-600">{{ listings.filter(l=>l.status==='pending').length }}</p></div>
      <div class="stat-card"><p class="text-sm text-earth-500 mb-1">Sold</p><p class="font-display text-3xl font-bold text-coffee-700">{{ listings.filter(l=>l.status==='sold').length }}</p></div>
    </div>
    ...
    <div v-if="loading" class="space-y-3">
      <div v-for="n in 3" :key="n" class="card p-5 animate-pulse flex gap-4">
        ...
        <div class="flex-1"><div class="h-4 bg-earth-100 rounded w-1/2 mb-2"></div><div class="h-3 bg-earth-100 rounded w-1/3"></div></div>
      </div>
    </div>
      ...
      <table class="w-full">
        ...
        <tbody>
          <tr v-for="l in listings" :key="l.id" class="hover:bg-earth-50 transition-colors">
            <td class="table-cell"><div><p class="font-medium text-earth-900">{{ l.title }}</p><p class="text-xs text-earth-400 mt-0.5">{{ formatDate(l.created_at) }}</p></div></td>
            <td class="table-cell hidden sm:table-cell text-earth-600">{{ l.origin }}</td>
            <td class="table-cell hidden md:table-cell"><span v-if="l.grade" class="badge-approved text-xs">{{ l.grade }}</span><span v-else class="text-earth-400 text-xs">—</span></td>
            ...
            <td class="table-cell"><span :class="statusBadge(l.status)">{{ l.status }}</span></td>
            <td class="table-cell text-right">
              <div class="flex justify-end gap-2">
                <button @click="openModal(l)" class="text-xs px-3 py-1.5 border border-earth-200 text-earth-600 rounded-sm hover:bg-earth-50">Edit</button>
                <button @click="confirmDelete(l)" class="text-xs px-3 py-1.5 border border-red-200 text-red-600 rounded-sm hover:bg-red-50">Delete</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4 overflow-y-auto">
      <div class="bg-white rounded-sm shadow-warm-lg w-full max-w-lg my-6">
        ...
        <form @submit.prevent="saveListing" class="p-5 space-y-4">
          ...
          <div><label class="input-label">Coffee Title</label><input v-model="form.title" required class="input-field" placeholder="Yirgacheffe Natural Grade 1" /></div>
          <div class="grid grid-cols-2 gap-4">
            <div><label class="input-label">Origin</label><input v-model="form.origin" required class="input-field" placeholder="Yirgacheffe, SNNPR" /></div>
            <div><label class="input-label">Grade</label><select v-model="form.grade" class="select-field"><option value="">Select grade</option><option>Grade 1</option><option>Grade 2</option><option>Grade 3</option><option>Grade 4</option><option>Grade 5</option></select></div>
            <div><label class="input-label">Process</label><select v-model="form.process" class="select-field"><option value="">Select process</option><option>Washed</option><option>Natural</option><option>Honey</option><option>Anaerobic Natural</option></select></div>
            <div><label class="input-label">Harvest Date</label><input v-model="form.harvest_date" type="date" class="input-field" /></div>
            <div><label class="input-label">Quantity (kg)</label><input v-model.number="form.quantity_kg" type="number" required min="1" class="input-field" placeholder="500" /></div>
            <div><label class="input-label">Price ($/kg)</label><input v-model.number="form.asking_price_per_kg" type="number" required min="0" step="0.01" class="input-field" placeholder="8.50" /></div>
          </div>
          <div><label class="input-label">Description</label><textarea v-model="form.description" rows="3" class="input-field resize-none" placeholder="Describe your coffee..."></textarea></div>
          <div class="flex gap-3 pt-2">
            <button type="button" @click="closeModal" class="btn-secondary flex-1 py-3">Cancel</button>
            <button type="submit" :disabled="saving" class="btn-primary flex-1 py-3">{{ saving ? 'Saving...' : (editing ? 'Update' : 'Submit') }}</button>
          </div>
        </form>
      </div>
    </div>
...
async function fetchListings() {
  ...
  error.value = ''
  try {
    const res = await listingsApi.myList()
...
async function saveListing() {
  ...
  try {
    if (editing.value) {
      const res = await listingsApi.update(editing.value, { ...form })
      const idx = listings.value.findIndex(l => l.id === editing.value)
      if (idx !== -1) listings.value[idx] = res.data.listing || res.data
    } else {
      const res = await listingsApi.create({ ...form })
      listings.value.unshift(res.data.listing || res.data)
    }
    closeModal()
  } catch (err) {
